...
- [ ] Feature Spec 拆分是独立受控操作；“推入 Feature Spec Pool”不得再次调用拆分 Skill。
...
- [ ] 推入 Feature Spec Pool 时，系统读取 Skill 已规划好的机器可读队列产物，按规划结果写入候选记录并创建调度队列；不得由代码解析文档依赖关系或重新执行 Feature 拆分。
...
THE SYSTEM SHALL 恢复未完成 Run、Running 任务、Scheduled 任务、Runner 心跳、Git worktree 状态、Codex session、最近 Execution Result 和 Project Memory。
...
WHERE 开发阶段自动执行任务适用
THE SYSTEM SHALL 默认使用 danger-full-access 和 approval=never，避免 Codex CLI 人工确认阻塞开发流。

验收：
- [ ] 默认 Runner 配置使用最大 Codex CLI sandbox 权限且不请求人工确认。
