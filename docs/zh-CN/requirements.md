...
SpecDrive AutoBuild 是一个以 Spec、Scheduler、Project Memory、Runner CLI Adapter 外部运行观测和内部任务状态机驱动，并通过 Dashboard 呈现状态的长时间自主编程系统。它的目标不是让 AI 一次性生成代码，而是让 AI 在可控、可恢复、可审计的工程流程中持续交付代码。
...
- Runner CLI Adapter：通过 active 编码 CLI 执行代码修改、测试、修复和结构化结果输出。
...
验收：
- [ ] 审计可从任务、Runner policy、执行结果、Status Check 和状态转换中追踪。
...
WHEN 用户查看任务看板
THE SYSTEM SHALL 在任务卡片展示标题、Feature、User Story、状态、依赖、计划时间、最近 Runner、Execution Result、测试状态、diff 摘要、风险等级和审批状态。
...
THE SYSTEM SHALL 恢复未完成 Run、Running 任务、Scheduled 任务、Runner 心跳、Git worktree 状态、CLI session、最近 Execution Result 和 Project Memory。
...
验收：
- [ ] 开发阶段默认 Runner Policy 使用 `danger-full-access` 和 `approval=never`。
...
- [ ] Runner Console 和 Spec Workspace 能展示 scheduler job、run id、workspace、skill phase、blocked reason 和最近执行结果。
...
THE SYSTEM SHALL 展示 Runner 在线状态、active CLI adapter、当前模型、当前 sandbox、approval policy、queue、最近日志和心跳状态，并支持暂停或恢复 Runner。
...
THE SYSTEM SHALL 调用 Codex CLI 对消息进行意图分类，识别意图类型（query_status、query_review、add_requirement、change_requirement、schedule_run、pause_runner、resume_runner、approve_review、reject_review、generate_ears、generate_hld、help、cancel、confirm、unknown），并在 Codex 不可用时退回基于关键词的规则分类。
...
WHEN Chat Interface 识别到高风险意图（schedule_run、pause_runner、resume_runner、approve_review、reject_review）
...
来源：`docs/zh-CN/vscode-codex-app-server-prd.md` REQ-VSC-001；`docs/zh-CN/vscode-app-plan.md` FEAT-016
优先级：Should
...
来源：`docs/zh-CN/vscode-codex-app-server-prd.md` REQ-VSC-002、REQ-VSC-003；`docs/zh-CN/vscode-app-plan.md` FEAT-016
优先级：Should
...
来源：`docs/zh-CN/vscode-codex-app-server-prd.md` REQ-VSC-004 至 REQ-VSC-006；`docs/zh-CN/vscode-app-plan.md` FEAT-017
优先级：Should
...
来源：`docs/zh-CN/vscode-codex-app-server-prd.md` 第 7.4 节；`docs/zh-CN/vscode-app-plan.md` FEAT-017
优先级：Must
...
来源：`docs/zh-CN/vscode-codex-app-server-prd.md` 第 7.3 节；`docs/zh-CN/vscode-app-plan.md` Interfaces And Contracts
优先级：Must
...
来源：`docs/zh-CN/vscode-codex-app-server-prd.md` REQ-VSC-003、第 7.6 节；`docs/zh-CN/vscode-app-plan.md` FEAT-019
优先级：Should
...
### REQ-080：提供 Codex app-server Adapter
来源：`docs/zh-CN/vscode-codex-app-server-prd.md` 第 7.7 节；`docs/zh-CN/vscode-app-plan.md` FEAT-018
优先级：Must

WHEN Runner 消费 `codex.app_server.run` Job
THE SYSTEM SHALL 连接或启动 `codex app-server`，完成 initialize/initialized、thread/start 或 thread/resume、turn/start 和事件流消费。
...
WHEN Codex app-server 发出 approval request
...
来源：`docs/zh-CN/vscode-codex-app-server-prd.md` REQ-VSC-007；`docs/zh-CN/vscode-app-plan.md` FEAT-020
优先级：Should
...
来源：`docs/zh-CN/vscode-codex-app-server-prd.md` REQ-VSC-017；用户指令“为 VS 插件开发独立的 Web UI，不要复用现在的 Web UI，核心关注任务调度和自动执行”
优先级：Must
...
验收：
- [ ] 默认 Runner 配置使用最大编码 CLI sandbox 权限且不请求人工确认。
- [ ] 默认 Runner 配置不得使用 bypass approvals；需要无确认执行时使用 `approval=never`。
...
| PRD 第 6.9 节 Codex Runner | REQ-037, REQ-038, REQ-039, REQ-065, REQ-066, REQ-068, REQ-080, REQ-081, REQ-082 | Runner 执行、CLI Adapter、Codex app-server Adapter、JSON 配置、workspace-aware Skill invocation 与安全策略 |
...
| PRD 第 8.11 节 VSCode IDE | REQ-074, REQ-075, REQ-076, REQ-077, REQ-078, REQ-079, REQ-080, REQ-081, REQ-082, REQ-083, REQ-084 | VSCode 工作区识别、Spec Explorer、文档交互、Task Queue、独立 Execution Workbench Webview、app-server Adapter、审批和 Diagnostics |
...
| M8：SpecDrive IDE | REQ-074, REQ-075, REQ-076, REQ-077, REQ-078, REQ-079, REQ-080, REQ-081, REQ-082, REQ-083, REQ-084 |
