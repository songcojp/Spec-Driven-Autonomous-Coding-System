...
THE SYSTEM SHALL 自动执行 technical-context、research-decision、architecture-plan、data-model、contract-design、quickstart-validation、task-slicing 和 spec-consistency-analysis 等 Skill，并在完成后进入 tasked。

验收：
- [ ] 任一计划阶段失败时 Feature 进入 Review Needed，并保留失败证据。
- [ ] quickstart-validation 在任务切片前验证实现路径可启动、可测试或可明确标记阻塞；spec-consistency-analysis 在计划完成前验证计划、数据模型、契约和任务切片与 Feature Spec 一致。
...
- [ ] 调度触发来源、触发时间和触发原因被记录到 Project Memory 和审计日志。
...
验收：
- [ ] 每次调度运行都能追踪触发模式、触发时间、触发来源、触发对象和调度结果。
...
| PRD 第 8 节 页面需求 | REQ-052, REQ-053, REQ-054, REQ-055, REQ-056, REQ-057, REQ-061 | UI 表面需求 |
...
| M7：Product Console | REQ-052, REQ-053, REQ-054, REQ-055, REQ-056, REQ-061 |
