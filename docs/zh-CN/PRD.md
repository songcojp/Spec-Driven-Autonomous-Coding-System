...
产品方向：Spec + Skill + Subagent 驱动的长时间自主编程系统

---

## 1. 产品定义

SpecDrive AutoBuild 是一个面向软件团队的长时间自主编程系统。系统以结构化 Spec 管理产品目标和验收标准，以 Skill 固化可复用工程方法，以 Subagent 隔离上下文并行处理任务，以 Project Memory 为 CLI 提供跨会话持久记忆，以 Codex Runner 执行代码修改、测试和修复，以看板状态机管理任务流转、审批、恢复和交付。
...
5. 系统将大任务拆分为上下文隔离、边界明确的 Subagent Run。
6. 每个 Subagent 只接收完成当前任务所需的最小上下文。
7. Codex Runner 执行代码修改、测试、修复、PR 生成。
8. Status Checker 自动判断任务完成、失败、阻塞或需要审批。
9. Dashboard State Machine 实时呈现任务状态和交付进度。
...
Skill 是系统可复用的工程能力。每个 Skill 必须声明：
...
Subagent 是一次上下文隔离的专用 Agent 执行单元。Subagent 不继承完整主上下文，只接收 Agent Run Contract 指定的任务目标、上下文切片、读写范围、验收标准和输出格式。
...
Project Memory 以结构化 Markdown 持久化，保存在 `.specdriver/memory/project.md`，并在每次 Run 结束后由系统自动更新。
...
phase: tasking
risk: medium
input_schema: TaskSlicingInput
output_schema: TaskGraphOutput
```
...
#### FR-022 Skill 输入输出 Schema

每个 Skill 必须使用结构化输入输出，便于 Orchestrator 调度、审计和重放。

Skill schema 必须满足：

* 输入和输出必须声明机器可验证 schema，MVP 优先采用 JSON Schema；TypeScript 实现可使用 Zod，Python 实现可使用 Pydantic，但必须能导出或映射为 JSON Schema。
* Orchestrator 在 Skill 执行前校验输入，在 Skill 返回后校验输出；校验失败不得进入下一阶段，必须生成 Evidence Pack 并进入 `review_needed` 或失败恢复流程。
* schema 必须包含 `schema_version`、必填字段、枚举值、数组元素类型、可空字段和向后兼容策略。
* Skill 注册信息中的 `input_schema` / `output_schema` 必须与运行时校验器一致，禁止只写文档不校验。

`pr-ears-requirement-decomposition-skill` 的最小输出 schema：

```json
{
  "skill": "pr-ears-requirement-decomposition-skill",
  "input": {
    "schema_version": "1.0.0",
    "source_type": "natural_language|pr|rp|prd|ears|mixed",
    "source_text": ""
  },
  "output": {
    "schema_version": "1.0.0",
...
Skill 支持版本号、变更记录、启用/禁用、项目级覆盖、团队级共享和版本回滚。
...
#### FR-031 Agent Run Contract

每个 Subagent 启动时必须生成 Agent Run Contract。
...
* 每个 worktree 必须绑定独立分支、任务/Feature 标识和合并目标，并在 Result Merger 合并前完成冲突检测。

#### FR-033 Subagent 结果合并

Result Merger 负责去重、冲突检测、风险合并、可信度评估、下一步动作生成和看板状态更新。**State Agent 在看板状态变更后触发 Project Memory 状态快照同步。**

### 6.5 Project Memory

#### FR-044 Project Memory 初始化

项目创建时系统初始化 Project Memory 文件 `.specdriver/memory/project.md`，内容包含项目名称、目标、默认分支、当前 Spec 版本、初始任务状态快照和空的运行记录。
...
* 将已完成任务移入完成列表
* **更新任务看板状态快照**（同步 Dashboard State Machine 的最新状态）
...
* 多 Feature 并行完成后必须通过 Result Merger 汇总 diff、检测冲突、合并 Evidence，并按目标分支顺序合并
...
系统记录上次失败原因、上次修复方案、禁止重复策略、失败次数和失败模式指纹。
...
审批页面展示任务目标、关联 Spec、Agent Run Contract、diff 摘要、测试结果、风险说明、推荐动作和可选操作。
...
支持查看 Skill 列表、详情、版本、输入输出 schema、启用状态、执行日志、成功率、适用阶段和风险等级。

### 8.4 Subagent Console

支持查看当前 Subagent、Run Contract、上下文切片、Evidence Pack、token 使用、运行状态，并支持终止和重试 Subagent。
...
* 高风险文件保护。
* `.env`、密钥、支付、认证配置需特殊保护。
* Subagent 的文件访问范围通过 Agent Run Contract 中的 `allowed_files`、`read_only_files` 和 `forbidden_actions` 字段传入 CLI，由 CLI 自身沙箱策略执行。
* 所有命令记录审计日志。
* 所有审批可追踪。
* 所有自动修改可回滚。
...
以 Dashboard State Machine 管理长时间自主交付。
```
