...
* 阶段 2 需求录入：将 Spec 来源扫描和上传合并为一个阶段内步骤，并在该步骤中显示“扫描”和“上传”两个动作；继续展示识别 PR/RP/PRD/EARS、生成 EARS / Feature Spec、完成关键澄清、执行需求质量检查、推入 Feature Spec Pool 的状态、阻塞原因和事实来源。
* 阶段 3 规划执行：展示 HLD 生成、Feature Spec 拆分、规划流水线和状态检查的状态、阻塞原因和事实来源。
