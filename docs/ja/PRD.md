...
製品の方向性: Spec + Skill + Subagent 主導の長時間自律型プログラミングシステム
...
5. 大規模なタスクを、コンテキストが分離され、境界が明確なSubagent Runに分割する。
...
### 4.2 Skill System
再利用可能なエンジニアリング能力。各Skillには名前、説明、トリガー、入力/出力スキーマ、リスクレベルなどが定義されている必要がある。
...
## 7. コアデータモデル
Project、Feature、Requirement、Task、Run、ProjectMemory、EvidencePack のJSONスキーマが含まれる。

---

## 8. ページ要件
Dashboard、Spec Workspace、Skill Center、Subagent Console、Dashboard Board、Runner Console、および Review Center のUI要件。
