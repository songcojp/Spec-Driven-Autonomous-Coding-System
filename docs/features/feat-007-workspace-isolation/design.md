...
|---|---|
| Worktree Allocator | 由实现技能在高权限 Runner sandbox 中为 Feature 或 Task 创建或验证隔离 worktree 和分支。 |
...
3. Runner 使用允许访问 Git 元数据和外部 worktree 路径的高权限 sandbox 启动实现技能。
4. 实现技能在目标项目仓库中创建或验证隔离 worktree，然后在 worktree 中执行。
