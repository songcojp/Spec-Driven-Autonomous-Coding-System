...
1. Project Scheduler 由立即执行、指定时间、周期巡检、依赖完成、CI 失败或审批通过等模式触发。
2. Feature Selector 从 Feature Spec Pool 动态计算 ready 候选。
3. 选中的 Feature 进入 `planning`。
4. Planning Pipeline 运行计划阶段 Skill，并在任务切片前完成 quickstart validation，在计划完成前完成 spec consistency analysis。
