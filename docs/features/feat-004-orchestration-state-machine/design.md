...
Scheduler and State Maintenance 是控制面核心。它从 Feature Spec Pool 读取候选，生成 Task Graph，维护 Feature/Task 状态机，并根据 Runner observation、Status Check、Review、Recovery 和 Delivery 结果推进状态。它不调用 Skill，不创建 Subagent，也不执行 Planning Pipeline。
...
- Owns: FeatureSelectionDecision、ScheduleTrigger、TaskGraph、Task、TaskSchedule、StateTransition、AuditTimelineEvent。
...
4. 选中的 Feature 进入 `planning` 或后续由外部计划成果推进的状态。
5. Task Graph Builder 按已批准的 Feature Spec 生成任务图，Feature 进入 `tasked`。
