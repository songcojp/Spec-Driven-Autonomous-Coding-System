...
- CLI skill invocation contract 必须包含 `projectId`、`workspaceRoot`、`skillSlug`、`sourcePaths`、`expectedArtifacts`、`traceability` 和 `requestedAction`。
...
- [ ] Spec/UI 操作可以生成 skill invocation prompt，并在 Evidence 中追踪 workspace、skill phase 和 expected artifacts。
- [ ] Runner Policy 能根据任务风险解析 sandbox、approval、model、profile 和输出 schema。
- [ ] CLI Adapter JSON 配置可以校验、保存草稿、启用，并在无效时阻塞新 Run。
- [ ] workspace root 缺失、不可读或缺少所需 Skill 文件时，新 Run blocked 且给出可观察原因。
- [ ] 默认 Runner 配置不使用危险权限。
