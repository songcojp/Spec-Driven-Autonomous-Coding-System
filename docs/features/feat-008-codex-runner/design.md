...
Codex Runner 是执行层入口。它由 BullMQ `cli.run` Worker 或 planning bridge 触发，读取 Run、Task/Feature、当前项目 workspace、Runner Policy 和 active CLI Adapter JSON 配置，在目标项目 workspace root 中调用 Codex CLI，采集事件、日志、心跳和结构化输出，并把结果交给 Evidence Store 和 Status Checker。Codex CLI 是 MVP 默认 adapter；命令模板、参数映射、输出解析和 session resume 规则由 adapter 配置承载。
...
1. Feature Scheduler 为已排期任务创建 Run 并 enqueue `cli.run` job，或 `feature.plan` bridge 为 planning 创建 Run。
...
4. Skill Invocation Prompt Builder 根据 `skillSlug`、`sourcePaths`、`expectedArtifacts` 和 `traceability` 生成 prompt。
5. Safety Gate 检查是否允许执行。
6. Codex CLI Adapter 在目标项目 workspace root 中运行。
7. Heartbeat 周期更新。
8. Raw Log Collector 归档脱敏日志和 JSON 事件。
9. Worker 持久化 session/log/evidence/status check，并按结果回写 Run 与 Task/Feature 状态。

workspace root 不得回退到 SpecDrive Control Plane 进程 cwd。路径缺失、不可读、不是可用 workspace，或缺少执行所需 `.agents/skills/*` / `AGENTS.md` 时，Runner 必须 blocked 并把原因写入 Run summary、Evidence 和 Runner Console。
...
- FEAT-004 通过 `feature.plan` bridge 提供 planning CLI run 入口。
...
- Skill invocation prompt 必须在 Evidence metadata 中追踪 `workspaceRoot`、`skillSlug`、`sourcePaths`、`expectedArtifacts` 和 `traceability`。
