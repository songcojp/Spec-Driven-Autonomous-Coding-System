...
Codex Runner 是执行层入口。它读取 Run、Task、WorktreeRecord、Runner Policy 和 active CLI Adapter JSON 配置，在指定 worktree 中调用 Codex CLI，采集事件、日志、心跳和结构化输出，并把结果交给 Evidence Store 和 Status Checker。Codex CLI 是 MVP 默认 adapter；命令模板、参数映射、输出解析和 session resume 规则由 adapter 配置承载。
...
8. 执行结果进入 Evidence Store 和 Status Checker。
