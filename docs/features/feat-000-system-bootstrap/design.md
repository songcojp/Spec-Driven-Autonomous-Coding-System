...
本 Feature 是 AutoBuild 系统的启动基础层。所有 FEAT-001 至 FEAT-014 的持久化、CLI Skill 发现和 artifact 存储能力都依赖本 Feature 完成初始化后才可使用。Bootstrap 在 Control Plane 进程启动阶段同步执行，完成后才允许 HTTP API、Runner Queue 和 Dashboard 接受请求。
...
| 项目本地 Skill 发现检查 | Skill 执行逻辑（Codex CLI） |
...
| REQ-011 | 5.4 Skill Discovery Check | Bootstrap 完成后确认项目本地 Skill 文件可发现；Skill 调用由 Codex CLI 负责。 |
...
flowchart TD
  ...
  Schema --> Seeder[Skill Discovery Check]
...
| `recovery_dispatches` | RecoveryDispatch |
...
### 5.4 Skill Discovery Check

Responsibilities:
- 扫描项目本地 `.agents/skills/*/SKILL.md`。
- 确认至少存在一个可用 Skill 文件。
- 不写入 SQL Skill Registry。
...
Responsibilities:
- 在 Config Loader、Artifact Dir Initializer、Schema Manager、Skill Discovery Check 全部成功后将系统状态设置为 `ready`。
...
sequenceDiagram
  ...
  participant SS as Skill Discovery Check
  ...
  Proc->>SS: discover project skills
  SS-->>Proc: project skills found
  Proc->>RG: set ready
...
sequenceDiagram
  ...
  Proc->>SS: scan .agents/skills → found
...
| Skill Discovery Check 失败 | 输出错误，不标记就绪，进程中止。 |
...
| 级别 | 覆盖点 |
|---|---|
| 单元 | Config Loader 合并逻辑、Skill Discovery Check、Schema 版本比对逻辑。 |
...
- **Skill 发现边界**：Bootstrap 只确认 `.agents/skills/*/SKILL.md` 存在；Skill 调用、上下文和执行契约由 Codex CLI 原生机制负责。
