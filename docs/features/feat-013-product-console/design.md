...
Product Console 是用户可访问的浏览器控制台，也是控制面状态的查询和命令入口。它由 Project Switcher、Dashboard、Project Home、Spec Workspace、Runner Console、Review Center 和 Language Switcher 组成，只通过 Control Plane API 查询和发起受控命令，不直接修改 Git 工作区。现有 Query/ViewModel 和 HTTP JSON endpoint 是后端契约，不是 UI 交付物。
