...
Product Console 是控制面状态的查询和命令入口。它由 Dashboard、Dashboard Board、Spec Workspace、Skill Center、Subagent Console、Runner Console 和 Review Center 组成，只通过 Control Plane API 查询和发起受控命令，不直接修改 Git 工作区。
...
4. Console Command Gateway 将拖拽、批量排期、批量运行、暂停、恢复、终止和重试等动作提交为受控命令。
