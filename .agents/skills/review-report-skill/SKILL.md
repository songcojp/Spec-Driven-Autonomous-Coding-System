...
description: "Produce review findings and delivery-risk reports for SpecDrive changes. Use when diff, test evidence, architecture risk, approval items, or Review Center records need concise actionable findings. Includes spec drift detection: behavior that diverges from REQ-* requirements is reported as a finding."
---
...
4. Anchor findings to file paths, requirement IDs, or evidence IDs. Every finding must state: location, expected behavior (from spec), actual behavior (from diff), and severity.
