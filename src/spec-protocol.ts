...
export type FileSpecFeatureType = "IDE-Webview" | "App" | "Library" | "Foundation" | "Service" | "Other";

export type UserHarnessRef = {
  /** Path to the generated agent-user-harness.md artifact */
  path: string;
  /** UH-### user scenario IDs that this Feature's implementation must satisfy */
  scenarioRefs: string[];
};

export type DeliveryHarnessRef = {
  /** Path to the delivery-harness.md artifact produced by design-ui-spec */
  path: string;
  /** F-### workflow IDs this Feature delivers */
  workflowRefs: string[];
  /** DHI-### design target IDs that anchor implementation to WYSIWYG design */
  dhiRefs: string[];
  /** TM-### acceptance oracle IDs that must run in verify-behavior */
  tmRefs: string[];
  /** Stitch MCP screen IDs generated by stitch-design-coverage */
  stitchScreenIds: string[];
  /**
   * Lifecycle status of the delivery harness for this Feature:
   *   pending   — design-ui-spec has not yet produced coverage artifacts
   *   ready     — DHI, TM, and stitchScreenIds are all populated; Feature may be scheduled
   *   gap       — coverage artifacts exist but have identified gaps (coverage_contract_gap)
   *   blocked   — Stitch MCP unavailable or design not yet approved
   */
  harnessStatus: "pending" | "ready" | "gap" | "blocked";
};
...
export type FileSpecState = {
  ...
  updatedAt: string;
  /** Classifies the Feature's primary surface for harness gate routing */
  featureType?: FileSpecFeatureType;
  /** Reference to the Agent User Harness artifact (UH-### user scenario map) */
  userHarnessRef?: UserHarnessRef;
  /** Reference to the Product Delivery Harness artifact (DHI/TM/Stitch coverage) */
  deliveryHarnessRef?: DeliveryHarnessRef;
  currentJob?: {
